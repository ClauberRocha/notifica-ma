...
export const Route = createFileRoute("/_authenticated/fichas/febre-amarela")({
  ...
  component: FichasFebreAmarelaPage,
});
...
const CLASSIF_LABEL: Record<string, string> = {
  febre_amarela_silvestre: "FA silvestre",
  febre_amarela_urbana: "FA urbana",
  descartado: "Descartado",
};

function FichasFebreAmarelaPage() {
      ...
      <div className="bg-card border rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <Button asChild>
              <Link to="/nova-ficha/febre-amarela"><FilePlus className="w-4 h-4 mr-1" /> Cadastrar primeira ficha</Link>
            </Button>
          </div>
        ) : (
          <Table>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id} className="cursor-pointer hover:bg-muted/50" onClick={() => { window.location.href = `/fichas/febre-amarela/${r.id}`; }}>
