...
export const Route = createFileRoute("/_authenticated/fichas/tetano-acidental/")({
  ...
  component: FichasTetanoAcidentalPage,
});
...
const EVOL_LABEL: Record<string, string> = {
  cura: "Cura",
  obito_tetano_acidental: "Óbito (tétano)",
  obito_outras_causas: "Óbito (outras causas)",
  ignorado: "Ignorado",
};

function FichasTetanoAcidentalPage() {
      ...
      <div className="bg-card border rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <Button asChild>
              <Link to="/nova-ficha/tetano-acidental"><FilePlus className="w-4 h-4 mr-1" /> Cadastrar primeira ficha</Link>
            </Button>
          </div>
        ) : (
          <Table>
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow key={r.id} className="cursor-pointer hover:bg-muted/50" onClick={() => { window.location.href = `/fichas/tetano-acidental/${r.id}`; }}>
