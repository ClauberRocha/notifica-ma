...
import {
  ...
  PROFILAXIA_KEYS,
  MANIFESTACOES_KEYS,
  MEDIDAS_CONTROLE_KEYS,
} from "@/lib/tetano-acidental-options";

export const Route = createFileRoute("/_authenticated/nova-ficha/tetano-acidental")({
  head: () => ({ meta: [{ title: "Nova Ficha de Tétano Acidental" }] }),
  component: NovaFichaTetanoAcidentalPage,
});
...
type Step = {
  ...
  description?: string;
  fields?: FieldDef[];
  custom?: "profilaxia" | "manifestacoes" | "medidas" | "antecedentes_epi" | "dados_clinicos";
};
...
const STEPS: Step[] = [
  ...
  {
    title: "Lesão e exposição",
    fields: [
      { name: "possivel_causa", label: "Possível causa", type: "select", options: POSSIVEL_CAUSA, col: 2 },
      { name: "local_lesao", label: "Local da lesão", type: "select", options: LOCAL_LESAO },
    ],
  },
  {
    title: "Situação vacinal",
    fields: [
      { name: "situacao_vacinal_doses", label: "Situação vacinal (doses)", type: "select", options: SITUACAO_VACINAL, col: 2 },
      { name: "data_ultima_dose", label: "Data da última dose", type: "date" },
    ],
  },
  {
    title: "Profilaxia pós-ferimento",
    description: "Marque as profilaxias adotadas após o ferimento.",
    custom: "profilaxia",
  },
  { title: "Manifestações clínicas", custom: "manifestacoes" },
  {
    title: "Origem e hospitalização",
    fields: [
      { name: "origem_caso", label: "Origem do caso", type: "select", options: ORIGEM_CASO, col: 2 },
  ...
  },
  {
    ...
    description: "Marque as medidas de controle adotadas.",
    custom: "medidas",
  },
  {
    title: "Conclusão",
...
function NovaFichaTetanoAcidentalPage() {
  ...
  const [profilaxia, setProfilaxia] = useState<Record<string, boolean>>({});
  const [manifestacoes, setManifestacoes] = useState<Record<string, string>>({});
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      payload.profilaxia_pos_ferimento = profilaxia;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold mt-2">Nova ficha de Tétano Acidental</h1>
      ...
      <div className="bg-card border rounded-2xl p-6">
        ...
        {current.custom === "profilaxia" && (
          <CheckGrid items={PROFILAXIA_KEYS} values={profilaxia} onChange={setProfilaxia} />
        )}
        {current.custom === "manifestacoes" && (
          <SimNaoGrid items={MANIFESTACOES_KEYS} values={manifestacoes} onChange={setManifestacoes} />
        )}
        {current.custom === "medidas" && (
          <CheckGrid items={MEDIDAS_CONTROLE_KEYS} values={medidas} onChange={setMedidas} />
        )}
