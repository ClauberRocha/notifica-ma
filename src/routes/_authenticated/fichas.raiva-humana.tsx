...
export const Route = createFileRoute("/_authenticated/fichas/raiva-humana")({
  head: () => ({ meta: [{ title: "Fichas — Raiva Humana" }] }),
  component: FichasRaivaHumanaPage,
});
...
function FichasRaivaHumanaPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .select("id, numero_ficha, nome_paciente, data_notificacao, status, classificacao_final, evolucao, created_at")
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold mt-2">Fichas — Raiva Humana</h1>
        </div>
        <Button asChild>
          <Link to="/nova-ficha/raiva-humana">
      ...
      <div className="bg-card border rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center">
            ...
            <Button asChild>
              <Link to="/nova-ficha/raiva-humana"><FilePlus className="w-4 h-4 mr-1" /> Cadastrar primeira ficha</Link>
            </Button>
          </div>
        ) : (
          <Table>
